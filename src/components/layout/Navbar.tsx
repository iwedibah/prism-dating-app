...
import { Moon, Sun, Sparkles, LayoutDashboard } from "lucide-react";
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 glass" style={{ borderBottom: "1px solid var(--border)" }}>
        ...
        {mobileOpen && (
          <div className="md:hidden glass px-5 py-4 flex flex-col gap-4"
            ...
            {links.map((l) => (
              <Link key={l.label} href={l.href} className="text-sm font-medium"
                style={{ color: "var(--text-muted)" }} onClick={() => setMobileOpen(false)}>
                {l.label}
              </Link>
            ))}
            {isLoggedIn ? (
              <Link href="/feed" className="btn-primary text-sm py-2.5 text-center"
              ...
              <>
                ...
                <Link href="/signup" className="btn-primary text-sm py-2.5 text-center">Join Free</Link>
              </>
            )}
          </div>
        )}
